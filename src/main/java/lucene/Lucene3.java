...
public class Lucene3 {
	...
	public static void main(String[] args) throws IOException {
		...
		writer.commit();
		DirectoryReader directoryReader = DirectoryReader.open(writer.getDirectory());
		...
		for (String sentence : strings) {
			...
			for (int i = 0; i < words.length; i++) {
				...
				TermQuery query = new TermQuery(new Term("data", getPrefix(words, i) + " " + words[i]));
				...
				TopDocs search = indexSearcher.search(query, 2);
				 ...
				 if(docs.length == 1){
					 ...
					 System.out.println(doc);
					 System.out.println();
					 writer.updateDocument(new Term("data", getPrefix(words, i) + " " + words[i]), doc);
					 writer.commit();
				 }
				 else
				 {
