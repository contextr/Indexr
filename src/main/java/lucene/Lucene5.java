...
@Transactional()
public class Lucene5 implements CommandLineRunner {
	...
	public static void main(String[] args) throws IOException {
		SpringApplication.run(Lucene5.class, args);
	}
		...
		for (String sentence : strings) {
			...
			for (int i = 0; i < words.length; i++) {
				...
				for (String string : prefix) {
					...
					Iterable<Input> findByPrefixAndNext = jpa.findByPrefixAndNext(string, words[i]);
					for (Input input : findByPrefixAndNext) {
						input.setFrequency(input.getFrequency() + 1);
						jpa.save(input);
						updated = true;
					}
